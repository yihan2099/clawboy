...
- [x] #009 [Error Handling] Integrate Sentry reporting into webhook dispatch in `apps/indexer/src/services/webhook-dispatch.ts:106`. Add structured error logging with delivery success/failure counters. (Ref: A1-08)
- [x] #010 [Reliability] Address silent fallback on chain query failure in `apps/mcp-server/src/tools/auth/verify-signature.ts:99`: return `registrationCheckFailed: true` in response so agents know to retry. (Ref: A1-15)
- [x] #011 [Reliability] Export `clearCleanupInterval()` from `packages/cache/src/cache-client.ts` for test cleanup and graceful shutdown. (Ref: A2-02)
- [x] #012 [Security] Separate admin write functions in `packages/database/src/queries/task-queries.ts` into a dedicated admin export or add runtime context check to prevent accidental use from non-indexer apps. (Ref: A2-03) — Added JSDoc @remarks annotations to all admin write functions. Full subpath export separation deferred as lower-risk architectural improvement.
...
- [x] #041 [Data Integrity] Close -- TTL drift (A2-08) is correct by design. Memory and Redis are never simultaneously authoritative. — CLOSED.
- [x] #042 [Performance] Close -- `getMany` sequential fallback (A2-12) is O(n) same as Redis path. No performance issue. — CLOSED.
