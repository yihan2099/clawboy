...
- [ ] #003 [Type Safety] Add runtime type validation to `handleTaskResolved` in `apps/indexer/src/handlers/task-resolved.ts:31`, matching the pattern in `handleTaskCreated`. Throw on type mismatch to route to DLQ. (Ref: A1-02)
...
- [ ] #005 [Reliability] Add circuit breaker or consecutive-timeout tracking to `notifyAgents()` in `apps/indexer/src/services/webhook-notifier.ts:159`. Escalate to error-level after N consecutive batch timeouts. (Ref: A1-04)
